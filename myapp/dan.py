...
def an():
    ...
    data=folder.get_file('April_Realtor.csv')
    ...
    data=folder.get_file('Nevada Redfin 2020.xlsx')
    ...
    data=folder.get_file('Nevada Sold 2020.csv')
    ...
    #sold_df = sold_df.dropna(subset = ['LMS-SALE PRICE', 'LOT ACREAGE'])
    sold_df = sold_df[(sold_df['LMS-SALE PRICE'] != 0.0) & (sold_df['LOT ACREAGE'] != 0.0)]
    sold_df = sold_df[(sold_df['ASSESSED TOTAL VALUE'] != 0.0)]
    sold_df['Sale Price per Acre'] = sold_df['LMS-SALE PRICE'].div(sold_df['LOT ACREAGE'])
    sold_df['Sale to Assessed Ratio'] = sold_df['LMS-SALE PRICE'].div(sold_df['ASSESSED TOTAL VALUE'])
    ...
    df['Sold Properties (2020)'] = solds
    df['Sold Properties to Total Properties (2020)'] = solds_per_prop
    ...
    df['Mean Sale to Assessed Ratio'] = mean_sale_to_assessed
    df['Median Sale to Assessed Ratio'] = median_sale_to_assessed
    df['Sale to Assessed Ratio Std. Dev.'] = std_sale_to_assessed


    df_normal = df[['Population Density', 'Residents per Property', 'Domestic Migration 2019', 'Weighted Domestic Migration', 'Days on Market (Realtor)', 'Days on Market (Redfin)', 'Avg. DOM', 'New List to Sold', 'Sold Properties to Total Properties (2020)', 'Percent Owners in-zip', 'Percent Owners in-State']]
    ...
    df_normal['Sold Properties to Total Properties (2020)'] *= -1
    ...
    rankings['Sold Properties to Total Properties Rank'] = df['Sold Properties to Total Properties (2020)'].rank(method = 'min', ascending = False)
    ...
    for index, row in df_normal.iterrows():
        ...
        norm_var.append(row['Sold Properties to Total Properties (2020)'])
    ...
    summary['Sold Properties (2020)'] = df['Sold Properties (2020)']
    summary['Sold to Total Properties (2020)'] = df['Sold Properties to Total Properties (2020)']
    ...
    folder = site.Folder('apxn/Analysis')
    res=str(dt_string)+"_nv_metrics"+".csv"
    res1=str(dt_string)+"_nv_summary"+".csv"
    res2=str(dt_string)+"_nv_ranking"+".csv"
    print(res)
    print(res1)
    print(res2)
